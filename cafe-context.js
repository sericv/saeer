...
(function (global) {
  ...
  /**
   * SaaS feature tier for catalog / loyalty / preorder limits.
   * Stored on cafes/{cafeId} as saasPlan: "free" | "pro".
   * Omitted or unknown → "pro" (grandfathers existing stores).
   * Billing plan stays in `plan` (trial | monthly | yearly) and is unchanged.
   */
  function resolveSaasPlan(d) {
    const raw = d && typeof d === "object" ? d : {};
    const v = raw.saasPlan != null ? String(raw.saasPlan).trim().toLowerCase() : "";
  ...
  function buildStoreSubscriptionState(d) {
    ...
    const saasPlan = resolveSaasPlan(raw);
    const isSaasFree = saasPlan === "free";
  ...
  global.resolveSaasPlan = resolveSaasPlan;
