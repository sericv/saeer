...
let storeBranches = [];
let activeBranchId = typeof BranchContext !== "undefined" ? BranchContext.DEFAULT_BRANCH_ID : "main";
let branchesHydrated = false;

function withCafeTenant(payload) {
  return Object.assign({}, payload, { cafeId });
}

function settingsScopedRef(scope, branchId) {
  const bid = branchId != null ? branchId : activeBranchId;
  const docId =
    typeof BranchContext !== "undefined"
      ? BranchContext.settingsDocId(cafeId, scope, bid)
      : `${cafeId}_${scope}`;
  return db.collection("settings").doc(docId);
}

async function readBranchSettingsDoc(scope, branchId) {
  const bid = branchId != null ? branchId : activeBranchId;
  ...
  if (cafeId === DEFAULT_CAFE_ID) {
    const leg = await db.collection("settings").doc(scope).get();
...
/** "free" | "pro" — from cafes/{id}.saasPlan (default pro). */
let customerSaasPlan = "pro";
...
function isCustomerSaasFree() {
  return customerSaasPlan === "free";
}
...
function applyCustomerStoreBrandingFromDoc(raw) {
  ...
  document.title = name ? `${line} — سير` : "سير | القائمة";
  if (typeof resolveSaasPlan === "function") {
    customerSaasPlan = resolveSaasPlan(d);
  } else {
    customerSaasPlan = "pro";
  }
  applyCustomerSaasPlanChrome();
}

function applyCustomerSaasPlanChrome() {
  const free = isCustomerSaasFree();
  const mainApp = document.getElementById("mainApp");
  const bottomNav = document.getElementById("bottomNav");
  if (mainApp) mainApp.classList.toggle("customer-saas-free", free);
  if (bottomNav) bottomNav.classList.toggle("customer-saas-free", free);
  document.documentElement.classList.toggle("customer-saas-free", free);
  ...
  const cartNav = document.getElementById("navCartItem");
  if (loyaltyNav) loyaltyNav.style.display = free ? "none" : "";
  if (profileNav) profileNav.style.display = free ? "none" : "";
  if (cartNav && free) {
    cartNav.classList.add("nav-item--hidden");
    cartNav.setAttribute("aria-hidden", "true");
  }

  const footer = document.getElementById("customerSaerFooter");
  if (footer) {
    footer.style.display = free ? "block" : "none";
    footer.setAttribute("aria-hidden", free ? "false" : "true");
  }

  const badge = document.querySelector("#menuCategoriesView .section-badge");
  if (badge) badge.textContent = free ? "القائمة" : "Premium";

  if (free && currentUserId) {
    logoutUser();
  }

  if (document.getElementById("loyaltyScreen")?.classList.contains("active") && free) {
    switchToScreen("home");
  }
  if (document.getElementById("profileScreen")?.classList.contains("active") && free) {
    switchToScreen("home");
  }
  if (document.getElementById("cartScreen")?.classList.contains("active") && free) {
    switchToScreen("home");
  }
...
function getVisibleProducts() {
  ...
  return list.filter(product => {
    ...
    return typeof BranchContext === "undefined" || BranchContext.productMatchesBranch(product, activeBranchId);
  });
}

function getActiveBranchRecord() {
  return storeBranches.find(b => b.id === activeBranchId) || null;
}

function renderBranchSwitcher() {
  const wrap = document.getElementById("branchSwitcherWrap");
  if (!wrap || typeof BranchContext === "undefined") return;
  const active = storeBranches.filter(b => b.status !== "inactive");
  if (active.length <= 1) {
  ...
  wrap.innerHTML = active
    .map(b => {
      const on = b.id === activeBranchId ? " is-active" : "";
      const label = (b.name || b.slug || "فرع").replace(/</g, "");
      return `<button type="button" class="branch-switch-btn${on}" data-branch-id="${b.id}" onclick="switchCustomerBranch('${b.id}')">${label}</button>`;
    })
    .join("");
  const sub = document.getElementById("storeSubtitleLine");
  const br = getActiveBranchRecord();
  if (sub && br) {
    const base = sub.dataset.baseLabel || sub.textContent || "متجر";
    if (!sub.dataset.baseLabel) sub.dataset.baseLabel = base;
    sub.textContent = `${base} · ${br.name || br.slug}`;
  }
}

async function switchCustomerBranch(branchId) {
  if (!branchId || branchId === activeBranchId) return;
  activeBranchId = branchId;
  try {
    localStorage.setItem(BranchContext.branchStorageKey(cafeId), branchId);
  } catch (e) {}
  const br = storeBranches.find(b => b.id === branchId);
  if (br && br.slug) {
    const url = new URL(window.location.href);
    url.searchParams.set("cafe", cafeId);
    if (br.slug !== BranchContext.DEFAULT_BRANCH_ID) url.searchParams.set("branch", br.slug);
...
async function initStoreBranches() {
  if (!db || typeof BranchContext === "undefined") return;
  try {
    storeBranches = await BranchContext.loadBranchesForCafe(db, cafeId);
...
window.switchCustomerBranch = switchCustomerBranch;
...
function renderProductsGrid() {
  ...
  const allowCartUi = !isCustomerSaasFree() && preOrderEnabled && preorderGeneralSettingsLoaded;
...
function openProductOptionsModal(productId, forceCartIntent) {
  ...
  const cartMode = !!(
    !isCustomerSaasFree() &&
    preOrderEnabled &&
    preorderGeneralSettingsLoaded &&
    isPreorderAvailableNow() &&
    !isCustomerPreorderCartLockedForUi()
  );
...
function ensureHomeBannerAutoplay() {
  ...
  const list = homeBannersDisplayList.length ? homeBannersDisplayList : homeBanners;
  if (list.length > 1) {
...
function goToHomeBanner(index) {
  ...
  homeBannerIndex = (index + list.length) % list.length;
...
function renderHomeBanners() {
  ...
  homeBanners = list;
  const displayBanners = isCustomerSaasFree() ? list.slice(0, 1) : list;
  homeBannersDisplayList = displayBanners;
  if (homeBannerIndex >= displayBanners.length) homeBannerIndex = 0;
  track.innerHTML = displayBanners.map((b, i) => `
  ...
  `).join("");
  dots.style.display = displayBanners.length > 1 ? "flex" : "none";
  dots.innerHTML = displayBanners.length > 1
    ? displayBanners.map((_, i) => `<button class="home-banner-dot ${i === homeBannerIndex ? "active" : ""}" onclick="window.__goToHomeBanner(${i})"></button>`).join("")
    : "";
...
function requireLoginForPreorder() {
  if (isCustomerSaasFree()) return false;
    ...
    await db.runTransaction(async (transaction) => {
      ...
      transaction.set(
        ...
        withCafeTenant({
          orderNumber,
          orderSeq: seqUsed,
          branchId: activeBranchId || BranchContext.DEFAULT_BRANCH_ID,
...
async function startMainApp() {
  if (db && typeof checkCafeSubscription === "function") {
    ...
    if (sub.saasPlan) {
      customerSaasPlan = sub.saasPlan;
    }
  }
  try {
    if (db && typeof resolveSaasPlan === "function") {
      const snap = await db.collection("cafes").doc(cafeId).get();
      customerSaasPlan = resolveSaasPlan(snap.exists ? snap.data() : {});
    }
  } catch (e) {}
  applyCustomerSaasPlanChrome();
  await initStoreBranches();
  showMainApp();
